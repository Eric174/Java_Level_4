...
public class Collider {
    ...
    public void checkCollisions() {
        ...
        for (int i = 0; i < gc.getUnitsController().getPlayerUnits().size(); i++) {
            AbstractUnit unit = gc.getUnitsController().getPlayerUnits().get(i);
            if (unit.getUnitType() == UnitType.HARVESTER && unit.getPosition().dst(gc.getUnitsController().getBase().getPosition()) < 30) {
                int value = unit.getContainer();
                if (value > 0) {
                    gc.getPlayerLogic().addMoney(value);
                    unit.setToZeroContainer();
                }
            }
        }
    }
