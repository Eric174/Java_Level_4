...
public class BattleMap {
    ...
    public List<Vector2> getResourceList() {
        resourceList.clear();
        for (int i = 0; i < COLUMNS_COUNT; i++) {
            for (int j = 0; j < ROWS_COUNT; j++) {
                if (cells[i][j].resource > 0) {
                    resourceList.add(new Vector2(cells[i][j].cellX * CELL_SIZE + CELL_SIZE / 2, cells[i][j].cellY * CELL_SIZE + CELL_SIZE / 2));
                }
            }
        }
        return resourceList;
    ...
    public Vector2 getBuildingCoordinates(BaseLogic logic) {
        Building building;
        for (int i = 0; i < COLUMNS_COUNT; i++) {
            for (int j = 0; j < ROWS_COUNT; j++) {
                building = getBuildingEntrance(i, j);
                if (building != null && building.getOwnerLogic() == logic) {
                    return new Vector2(cells[i][j].cellX * CELL_SIZE + CELL_SIZE / 2, cells[i][j].cellY * CELL_SIZE + CELL_SIZE / 2);
                }
            }
        }
        return null;
