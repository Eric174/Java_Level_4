...
public class Tank extends GameObject implements Poolable {
    ...
    public void update(float dt) {
        if (selected && Gdx.input.isButtonPressed(Input.Buttons.RIGHT)) {
    ...
    public void updateWeapon(float dt) {
        if (weapon.getType() == Weapon.Type.HARVEST) {
            if (gc.getMap().getResourceCount(this) > 0) {
                ...
                if (result > -1) {
                    if (container < maxContainerSize) {
                        container += gc.getMap().harvestResource(this, result);
                        // Если собрали ресурсов больше, чем наш контейнер, то предотвращаем это
                        // (возможна ситуация: у нас уже есть 49 ресурсов, собираем еще 5, но у нас максимум = 50)
                        if (container > maxContainerSize) {
                            container = maxContainerSize;
                        }
                    }
                }
            } else {
    ...
    public void render(SpriteBatch batch) {
        ...
        if (weapon.getType() == Weapon.Type.HARVEST && weapon.getUsageTimePercentage() > 0.0f) {
            ...
            batch.draw(progressbarTexture,position.x - 32, position.y - 40, 64 ,12);
            batch.setColor(0.5f, 0.0f, 0.0f, 1.0f);
            batch.draw(progressbarTexture,position.x - 30, position.y - 38, (60 * container) / maxContainerSize,8);
            batch.setColor(1.0f, 1.0f, 1.0f, 1.0f);
        }
    }
}
